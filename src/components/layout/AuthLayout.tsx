...
export function AuthLayout({ children, title = 'OnlyFans Sales' }: AuthLayoutProps) {
  return (
    <div className="min-h-screen dark:bg-dark-bg bg-light-bg dark:text-white text-light-text flex flex-col transition-colors duration-200">
      {/* Logo area */}
      <div className="flex-1 flex flex-col items-center justify-center px-4 sm:px-6 pt-8 sm:pt-12">
        ...
        {/* Title */}
        <h1 className="text-xl sm:text-2xl font-bold dark:text-white text-light-text mb-2">{title}</h1>
        <p className="text-[#8B8B9E] text-center mb-8">Your sales, your numbers, your empire</p>

        {/* Form Card */}
        <div className="w-full max-w-[420px] sm:max-w-[480px] md:max-w-[540px] dark:bg-dark-card bg-light-card dark:border-dark-border border-light-border rounded-2xl sm:rounded-3xl p-5 sm:p-6 md:p-8 dark:text-white text-light-text transition-colors duration-200">
          {children}
        </div>
      </div>
